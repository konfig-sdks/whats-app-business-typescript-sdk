...
import { readableStreamToString, WhatsAppBusinessError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new WhatsAppBusinessError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new WhatsAppBusinessError(e, e.response?.data)
                }
                if (innerError instanceof WhatsAppBusinessError) {
                    // Got "WhatsAppBusinessError" from the above try block
                    throw innerError;
                }
